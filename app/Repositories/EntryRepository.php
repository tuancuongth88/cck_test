...
class EntryRepository extends BaseRepository implements EntryRepositoryInterface
{
    ...
    public function findOne($id)
    {
        ...
        foreach ($files as $value){
            ...
            if ($value['item_type'] == OTHER_FILE ){
                $fileHR['other_file'][] = [
                    'link_dowload' => url('download-fidelity/'.$value['id']),
                    'file_name' => $value['file_name'],
                ];
            }
        }
    ...
    public function list($attributes, $paginate = true)
    {
        ...
        $items = $this->select('*', "$mainTbl.id", "$mainTbl.updated_at", "$mainTbl.status")
            ->join('hrs', 'hrs.id', '=', "$mainTbl.hr_id")
            ->join('works', 'works.id', '=', "$mainTbl.work_id");
        $ids = $attributes['ids'] ?? '';
        if ($ids) {
    ...
    public function updateStatus($attributes, $id)
    {
        try {
            ...
            $updateStatus = $entry->update([
                'status' => $status,
                'note' => $note,
                'display' => $status == ENTRY_STATUS_CONFIRM ? 'off' : 'on'
            ]);
            //tạo mới thông tin ở interview
            if ($status == ENTRY_STATUS_CONFIRM && in_array($authLogin->type, [COMPANY, COMPANY_MANAGER])) {
    ...
    public function hide($ids)
    {
        try {
            ...
            Entry::query()->whereIn('id', $arrayListIdEntry)->update(['display' => 'off',]);
            return true;
        } catch (\Exception $exception) {
            Log::error($exception);
            return false;
