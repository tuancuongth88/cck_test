...
//        $listNameJa = $this->getListNameJa('f');
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_ADJUSTMENT_ADJUSTING, $dataNameHr);
    ...
    private function sendNotificationInterviewAdjustmentUrlSettingForManager($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_URL_SETTING_MANAGER);
    ...
    private function sendNotificationInterviewAdjustmentUrlSettingForCompany($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_URL_SETTING_COMPANY);
    ...
    private function sendNotificationInterviewCancel($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_CANCEL);
        }
    }
    ...
    private function sendNotificationInterviewDecline($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_DECLINE);
        }
    }
    ...
    private function sendNotificationInterviewAdjustmentAdjusted($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_ADJUSTMENT_ADJUSTED);
        }
    }
    ...
    private function sendNotificationInterviewPass($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_PASS);
    ...
    private function sendNotificationInterviewOffer($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_OFFER);
    ...
    private function sendNotificationInterviewNoPass($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_NO_PASS);
        }
    }
    ...
    private function sendNotificationInterviewHrReject($company, $hrOrg, $interview, $status, $note = null)
    {
        ...
        foreach ($users as $user) {
            $this->sendNotification($user, $company, $interview, self::TYPE_NOTIFY_INTERVIEW_HR_REJECT);
        }
    }
...
//        $getTimeFollowJA = Common::getTimeFollowJA($dataCalendarEnd['date'], $dataCalendarEnd['start_time'], $dataCalendarEnd['end_time'], $dataCalendarEnd['expected_time'], $dataCalendarEnd['at_time'], $dataCalendarEnd['weekdays']);
    ...
    private function sendNotification($user, $company, $interview, $typeNoti, $listNameJa = [])
    {

        $data['permission'] = User::getPermissionName($user);
        $data['type'] = $user->type;
        $data['type_noti'] = NOTI_TYPE_INTERVIEW;
        ...
        $data['status'] = $interview->status_selection;
        $data['status_text'] = STATUS_SELECTIONS[$interview->status_selection];
        $data['status_adjustment'] = $interview->status_interview_adjustment;
        $data['status_adjustment_text'] = STATUS_ADJUSTINGS[$interview->status_interview_adjustment];
