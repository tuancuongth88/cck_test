...
describe('Test component Offer Detail (Company)', () => {
  const fieldsOffer = [
    {
      key: 'selected',
      sortable: false,
  ...
  test('Case 5: Check With Role (Company) Can only see detail ', async() => {
    const wrapper = mount(offerOop, {
      localVue,
      router,
      store,
    ...
    // 4 Role (Company) 2 company_manager
    const params = {
      mail_address: '2okuridashi_hanoi@gmail.vn', // 4 company || 2 company_manager
      password: '123456789CCk',
    };
    ...
    if (
      ROLE.TYPE_COMPANY_ADMIN === permissionCheck ||
						ROLE.TYPE_COMPANY === permissionCheck
    ) {
      ...
      // Tìm modal cha chỉ xem cho quyền 2 company_manager và 4 company
      const modalOfferModalDecline = wrapper.findAll('#offer-modal-decline');
      expect(modalOfferModalDecline.exists()).toBe(true);
    }
  });
