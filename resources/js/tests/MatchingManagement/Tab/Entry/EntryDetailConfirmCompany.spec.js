...
describe('Test component Tab Entry Detail', () => {
  const fieldsEntry = [
    {
      key: 'selected',
      sortable: false,
  ...
  test('Case 5: Check modal entry requesting Confirm', async() => {
    const localVue = createLocalVue();
    const wrapper = mount(EntryOop, {
      localVue,
      router,
    ...
    const ModalRequestingElement = wrapper.find('#entry-modal-requesting');
    ...
    const entryModalRequestingConfirm = wrapper.find('#entry-modal-requesting-confirm');
