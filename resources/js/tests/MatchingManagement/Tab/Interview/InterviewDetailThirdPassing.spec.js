...
describe('Test component MatchingManagement Tab Entry', () => {
  ...
  test('Case 3: Check code 200 has data First passing', async() => {
  ...
  test('Case 4: Check render interview First passing - before adjustment data correct', async() => {
    const wrapper = mount(InterviewOop, {
      localVue,
      router,
      store,
  ...
  test('Case 5: Check document passing - before adjustment Setup Calender ', async() => {
    ...
    console.log('Case 5: codeGetListEntry: ', codeGetListEntry);
    ...
    // console.log('case 5: codeEntryConfirm: ', codeEntryConfirm);
    ...
    const dataGetListOffer = resGetListOffer.data.data.result;
    // console.log('case 5: dataGetListOffer[0].id :', dataGetListOffer[0].id);

    // Offer confirm
    const formDataOfferConfirm = {
      id: dataGetListOffer[0].id,
      status: 3, //
    };
    ...
    dataInterviewApiGlobal = dataInterviewApi;
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Status 1: before -> under

    // - Document passing before adjustment
    // status_interview_adjustment = 1
    // status_selection = 2

    // - Offer confirm before adjustment
    // status_selection = 1 //
    // status_interview_adjustment = 1
    // Before adjustment => Under adjustment
    ...
    // -----------------------------------------------------------------------
    const permissionCheck = store.getters.permissionCheck; // role = 1, 3, 5 (Ad, HrO, HR)
    const listRole = [1, 3, 5];
    // Status 2: Under -> Setting URL
    if (listRole.includes(permissionCheck)) {
    ...
    if (listRoleSettingURL.includes(permissionCheck)) {
  ...
  test('Case 6: Check modal Interview status Third passing works properly', async() => {
    ...
    expect(beforeAdjustmentModal.exists()).toBe(true);

    // Adjusted Done ==> Third Pass
    dataInterviewApiGlobal.map(item => {
      if (item.status_selection === 5 && item.status_interview_adjustment === 3) {
