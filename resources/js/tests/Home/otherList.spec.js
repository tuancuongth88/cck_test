...
describe('TEST new message Screen ', () => {
  ...
  const listRole = [2, 3, 4, 5];
  let role = 2;

  test('Case 1: Check render Other List Template', async() => {
  ...
  test('Case 2: Check call API Other List', async() => {
    const localVue = createLocalVue();
    const wrapper = shallowMount(newMsglistOop, {
      localVue, store, router,
    });
    ...
    role = Number(store.getters.profile.type); // 2, 3, 4, 5
  ...
  test('Case 3: Check render components Other List', async() => {
    const localVue = createLocalVue();
    const wrapper = shallowMount(newMsglistOop, {
      localVue, store, router,
    });
    ...
    if (listRole.includes(role)) {
      const newMsgOtherTable = wrapper.find('#new-msg-other');
      expect(newMsgOtherTable.exists()).toBe(true);
    }
