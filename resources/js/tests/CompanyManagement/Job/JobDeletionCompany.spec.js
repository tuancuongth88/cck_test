...
describe('TEST Component  ', () => {
  ...
  test('Case 2: Check render component btn Batch delete checked items', async() => {
    const localVue = createLocalVue();
    const wrapper = shallowMount(JobListCompanyOop, {
      localVue,
      store,
    ...
    await wrapper.setData({ fields: fields });
    ...
    await wrapper.setData({ items: arrListJob });
    ...
    await wrapper.setData({ pagination: pagination });
    expect(wrapper).toMatchSnapshot();

    console.log('Case 2 wrapper.vm.fields: ', wrapper.vm.fields);
  ...
  test('Case 3: Check render function handleDeleteAllSelectedCheckbox', async() => {
    const localVue = createLocalVue();
    const wrapper = shallowMount(JobListCompanyOop, {
      localVue,
      store,
    ...
    const funchandleDeleteAllSelectedCheckbox =
    jest.spyOn(wrapper.vm, 'handleDeleteAllSelectedCheckbox');
    await btnDeleteAllSelectedCheckbox.trigger('click');
    expect(funchandleDeleteAllSelectedCheckbox).toHaveBeenCalled();
