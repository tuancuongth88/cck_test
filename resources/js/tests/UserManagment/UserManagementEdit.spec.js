...
import UserManagementEdit from '@/pages/UserManagement/Edit.vue';
import { test } from '@jest/globals';

const CONPONENT_NAME = 'UserManagementCreate';
...
describe(`COMPONENT: ${CONPONENT_NAME}`, () => {
  ...
  test('Case 5: Test component render FORM EDIT', () => {
  ...
  test('Case 7: Test component render button Edit', () => {
    const wrapper = mount(UserManagementEdit);

    const BUTTON_EDIT = wrapper.find('.btn-sign-up');
    expect(BUTTON_EDIT.exists()).toBe(true);
    expect(BUTTON_EDIT.find('button').text()).toEqual('USER_MANAGEMENT_EDIT_BUTTON_SAVE');

    wrapper.destroy();
  });

  test('Case 8: Test component when click button Edit', async() => {
    const localVue = createLocalVue();
    const wrapper = mount(UserManagementEdit, {
      localVue,
      router,
    ...
    console.log(wrapper.vm.User.name);
