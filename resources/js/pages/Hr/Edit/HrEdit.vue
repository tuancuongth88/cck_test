    ...
    <div class="hr-basic-information">
      <div class="hr-basic-information-wrap">
        <div class="hr-basic-information-avata">
          <img :src="require('@/assets/images/login/hr-basic-info-avata-default.png')" alt="avata default">
        </div>
        ...
        <div class="hr-basic-information-general">
          ...
          <div class="w-100 information-general-select-file">
            <div class="d-flex flex-row align-items-center">
              <span style="line-height: 21px; font-size: 14px;" class="mr-2">履歴書</span>
              <Require />
            </div>

            <div class="w-100 d-flex flex-row align-items-center justify-content-between">
              <b-button v-if="!cv_file" :class="error.cv_file === false ? 'file-btn is-invalid-button' : 'file-btn'" @click="handleOpenFileSelect(1)">
                <span class="file-btn-text">ファイルを選択</span>
              </b-button>

              <input ref="fileInputCV" type="file" style="display: none;" @change="handleFileSelect(1, $event)">

              <span :class="[cv_file ? 'file-name-active' : 'ml-2 file-name']" @click="handlePreview(1)">{{ cv_file_name }}</span>

              <i v-if="cv_file" class="far fa-file-minus" @click="handleRemoveFile(1)" />
            </div>

            <div class="w-100 d-flex flex-row align-items-center justify-content-between">
              <b-form-invalid-feedback :state="error.cv_file">
                <span style="font-size: 10px;">{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
              </b-form-invalid-feedback>
            </div>

            <div class="d-flex flex-row align-items-center mt-3">
              <span style="line-height: 21px; font-size: 14px;" class="mr-2">職務経歴書</span>
              <Require />
            </div>

            <div class="w-100 d-flex flex-row align-items-center justify-content-between">
              <b-button v-if="!jd_file" :class="error.jd_file === false ? 'file-btn is-invalid-button' : 'file-btn'" @click="handleOpenFileSelect(2)">
                <span class="file-btn-text">ファイルを選択</span>
              </b-button>

              <input ref="fileInputJD" type="file" style="display: none;" @change="handleFileSelect(2, $event)">

              <span :class="[jd_file ? 'file-name-active' : 'ml-2 file-name']" @click="handlePreview(2)">{{ jd_file_name }}</span>

              <i v-if="jd_file" class="far fa-file-minus" @click="handleRemoveFile(2)" />
            </div>
            ...
            <div class="w-100 d-flex flex-row align-items-center mt-3">
              <span style="line-height: 21px; font-size: 14px;" class="mr-2">その他書類</span>
              <Arbitrarily />
            </div>

            <div class="w-100 d-flex flex-row align-items-center mt-1 button-add-more-file" @click="handleAddOtherFile()">
              <span style="color: #8E8E8E" class="file-name">ファイルを追加する</span>
              <i class="fas fa-plus-circle ml-2" />
            </div>

            <div class="d-flex flex-row align-items-center justify-content-between">
              <b-button v-if="!default_other_file" class="file-btn" @click="handleOpenFileSelect(3)">
                <span class="file-btn-text">ファイルを選択</span>
              </b-button>

              <input :ref="`fileInputDefaultOtherFile`" type="file" style="display: none;" @change="handleFileSelect(3, $event)">

              <span :class="[default_other_file ? 'file-name-active' : 'ml-2 file-name']" @click="handlePreview(3)">{{ default_other_file_name }}</span>

              <i v-if="default_other_file" class="far fa-file-minus" @click="handleRemoveFile(3)" />
            </div>

            <div v-for="(item, index) in other_files" :key="index" class="d-flex flex-row align-items-center justify-content-between">
              <b-button v-if="!item['file']" class="file-btn" @click="handleOpenFileSelect(4, index)">
                <span class="file-btn-text">ファイルを選択</span>
              </b-button>

              <input :ref="`fileInputOtherFile${index}`" type="file" style="display: none;" @change="handleFileSelect(4, $event, index)">

              <span :class="[item['file'] ? 'file-name-active' : 'ml-2 file-name']" @click="handlePreview(4, index)">{{ item['file_name'] }}</span>
      ...
      <div class="hr-detail-information_wrap">
        <div class="hr-detail_information-head">
          <div class="hr-basic-head-title">
            ...
            <span>{{ $t('HR_LIST_FORM.EDIT_FORM_TITLE') }} </span>
          </div>

          <div class="hr-detail-head-btns">
            <div class="btn btn-light" @click="handleCancelHrEdit">
              <span>{{ $t('BUTTON.CANCEL') }}</span>
            </div>

            <div class="btn btn-light btn-bold" @click="handleSaveHr()">
        ...
        <div class="hr-detail_information-tabs">
          <b-card no-body>
            <b-tabs card>
              <b-tab title="基本情報" active :title-link-class="isPassFirstTabValidation ? '' : 'error'">
                <b-card-text>
                  <div class="hr-content-tab">
                    <div class="hr-content-tab-wrap">
                      <div class="hr-content-tab-item  border-t border-l border-r">
                        ...
                        <div :class="['hr-content-tab__data d-flex flex-column', error.full_name ? '' : 'mt-3']">
                          <b-form-input
                            v-model="full_name"
                            type="text"
                            :class="error.full_name === false ? ' is-invalid' : ''"
                            @input="handleReAssignFeedback('full_name', $event)"
                          />
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r">
                        ...
                        <div :class="['hr-content-tab__data d-flex flex-column', error.full_name_furigana ? '' : 'mt-3']">
                          <b-form-input
                            v-model="full_name_furigana"
                            :class="error.full_name_furigana === false ? ' is-invalid' : ''"
                            @input="handleReAssignFeedback('full_name_furigana', $event)"
                          />

                          <b-form-invalid-feedback :state="error.full_name_furigana">
                            <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                          </b-form-invalid-feedback>
                        </div>
                      </div>
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r">
                        ...
                        <div class="hr-content-tab__data">
                          <b-form-select v-model="gender" :options="gender_option" :disabled-field="'disabled'" />
                        </div>
                      </div>
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r">
                        ...
                        <div class="hr-content-tab__data">
                          <div
                            :class="['d-flex justify-space-between flex-column', error.date_of_birth_year ? '' : 'mt-3']"
                            style="width: 160px; gap: 0.5rem; align-items: center;"
                          >
                            <div class="d-flex flex-row w-100 align-items-center">
                              <b-form-select
                                v-model="date_of_birth_year"
                                :options="date_of_birth_year_option"
                                :class="error.date_of_birth_year === false ? ' is-invalid' : ''"
                                @change="
                                  handleRenderDayInMonth(date_of_birth_year, date_of_birth_month),
                                  handleReAssignFeedback('date_of_birth_year', $event)
                                "
                              />
                            ...
                            <b-form-invalid-feedback :state="error.date_of_birth_year">
                              <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                            </b-form-invalid-feedback>
                          </div>

                          <div
                            :class="['d-flex justify-space-between flex-column ml-4', error.date_of_birth_month ? '' : 'mt-3']"
                            style="width: 160px; gap: 0.5rem; align-items: center;"
                          >
                            <div class="d-flex flex-row w-100 align-items-center">
                              <b-form-select
                                ...
                                :disabled="date_of_birth_year === '' ? true : false"
                                :class="error.date_of_birth_month === false ? ' is-invalid' : ''"
                                @change="
                                  handleRenderDayInMonth(date_of_birth_year, date_of_birth_month),
                                  handleReAssignFeedback('date_of_birth_month', $event)
                                "
                              />
                            ...
                            <b-form-invalid-feedback :state="error.date_of_birth_month">
                              <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                            </b-form-invalid-feedback>
                          </div>

                          <div
                            :class="['d-flex justify-space-between flex-column ml-4', error.date_of_birth_day ? '' : 'mt-3']"
                            style="width: 160px; gap: 0.5rem; align-items: center;"
                          >
                            <div class="d-flex flex-row w-100 align-items-center">
                              <b-form-select
                                ...
                                :disabled="date_of_birth_month === '' ? true : false"
                                :class="error.date_of_birth_day === false ? ' is-invalid' : ''"
                                @change="handleReAssignFeedback('date_of_birth_day', $event)"
                              />

                              <span class="d-flex ml-3">日</span>
                            </div>

                            <b-form-invalid-feedback :state="error.date_of_birth_day">
                              <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                            </b-form-invalid-feedback>
                          </div>
                        </div>
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r">
                        ...
                        <div class="hr-content-tab__data">
                          <b-form-select v-model="work_form" :options="work_form_option" :disabled-field="'disabled'" />
                        </div>
                      </div>
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r">
                        ...
                        <div class="hr-content-tab__data">
                          <div class="d-flex justify-space-between" style=" align-items: center;">
                            ...
                            <b-form-input v-model="work_form_part_time" :disabled="work_form === 1 || work_form === null" />
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r border-b">
                        ...
                        <div :class="['hr-content-tab__data d-flex flex-column', error.japanese_level ? '' : 'mt-3']">
                          <b-form-select
                            v-model="japanese_level"
                            :options="japanese_level_options"
                            :disabled-field="'disabled'"
                            :class="error.japanese_level === false ? ' is-invalid' : ''"
                            @change="handleReAssignFeedback('japanese_level', $event)"
                          />
              ...
              <b-tab title="学歴・職歴" :title-link-class="isPassSecondTabValidation ? '' : 'error'">
                <b-card-text>
                  <div class="hr-content-tab">
                    <div class="hr-content-tab-wrap">
                      <div class="hr-content-tab-item border-t border-l border-r">
                        <div id="hr-type-edit" class="hr-content-tab-item__title d-flex justify-start align-start">
                          <span>{{ $t('HR_LIST_FORM.FINAL_EDUCATION') }}</span>
                          <Require />
                        </div>

                        <div class="hr-content-tab__data">
                          <div class="w-100 d-flex flex-column py-3" style="gap: 1.25rem">
                            <div class="w-100 d-flex justify-space-center align-center">
                              <span style="width: 100%; max-width: 15%; font-size: 14px;" class="'pl-1">
                                ...
                                <div class="d-flex flex-column">
                                  <div class="d-flex flex-row align-items-center">
                                    <b-form-select
                                      ...
                                      :options="date_of_birth_year_option"
                                      :class="error.final_education_timing_year === false ? ' is-invalid' : ''"
                                      @change="handleReAssignFeedback('final_education_timing_year', $event)"
                                    />

                                    <span class="ml-3" style="font-size: 12px;">{{ $t('HR_LIST_FORM.YEAR') }}</span>
                                  </div>

                                  <b-form-invalid-feedback :state="error.final_education_timing_year">
                                    <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                  </b-form-invalid-feedback>
                                </div>
                                ...
                                <div class="d-flex flex-column">
                                  <div class="d-flex flex-row align-items-center">
                                    <b-form-select
                                      ...
                                      :class="error.final_education_timing_month === false ? ' is-invalid' : ''"
                                      @change="handleReAssignFeedback('final_education_timing_month', $event)"
                                    />

                                    <span class="ml-3" style="font-size: 12px;">{{ $t('HR_LIST_FORM.MONTH') }}</span>
                                  </div>

                                  <b-form-invalid-feedback :state="error.final_education_timing_month">
                                    <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                  </b-form-invalid-feedback>
                                </div>
                              </div>
                              ...
                              <div class="w-100 d-flex justify-space-between align-center">
                                <div class="w-100 d-flex flex-column align-items-center">
                                  <b-form-select
                                    ...
                                    :class="error.final_education_classification === false ? ' is-invalid' : ''"
                                    @change="handleReAssignFeedback('final_education_classification', $event)"
                                  />

                                  <b-form-invalid-feedback :state="error.final_education_classification">
                                    <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                  </b-form-invalid-feedback>
                                </div>
                              </div>
                              ...
                              <div class="w-100 d-flex justify-space-between align-center">
                                <div class="w-100 d-flex flex-column align-items-center">
                                  <b-form-select
                                    ...
                                    :class="error.final_education_degree === false ? ' is-invalid' : ''"
                                    @change="handleReAssignFeedback('final_education_degree', $event)"
                                  />

                                  <b-form-invalid-feedback :state="error.final_education_degree">
                                    <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                  </b-form-invalid-feedback>
                                </div>
                              </div>
                              ...
                              <div class="w-100 d-flex justify-space-between align-center">
                                <div class="w-100 d-flex flex-column align-items-center">
                                  <b-form-select
                                    ...
                                    :options="major_classification_options_api"
                                    :class="error.major_classification === false ? ' is-invalid' : ''"
                                    @change="handleReAssignFeedback('major_classification', $event)"
                                  />

                                  <b-form-invalid-feedback :state="error.major_classification">
                                    <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                  </b-form-invalid-feedback>
                                </div>
                              </div>
                              ...
                              <div class="w-100 d-flex justify-space-between align-center">
                                <div class="w-100 d-flex flex-column align-items-center">
                                  <b-form-select
                                    ...
                                    :options="middle_classification_options_api"
                                    :class="error.middle_classification === false ? ' is-invalid' : ''"
                                    :disabled="major_classification === null ? true : false"
                                    @change="handleReAssignFeedback('middle_classification', $event)"
                                  />

                                  <b-form-invalid-feedback :state="error.middle_classification">
                                    <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                  </b-form-invalid-feedback>
                                </div>
                              </div>
                          ...
                          <div class="w-100 d-flex flex-column py-3" style="gap: 1.25rem">
                            <div class="w-100 d-flex justify-space-center align-center">
                              <div class="w-100 d-flex justify-space-between align-center">
                                ...
                                <div class="w-100 d-flex flex-row justify-content-center align-items-center" style="gap: 0.75rem;">
                                  <div class="w-100 d-flex justify-start align-center">
                                    <div class="w-100 d-flex justify-space-between align-center" style="gap: 0.5rem;">
                                      <div class="w-100 d-flex flex-column" style="width: 54%; gap: 0.25rem;">
                                        <div class="w-100 d-flex flex-row align-items-center">
                                          <b-form-select
                                            v-model="main_job_career_1_year_from"
                                            :options="date_of_birth_year_option"
                                            style="width: 80px;"
                                            :class="error.main_job_career_1_year_from === false ? 'is-invalid' : ''"
                                            @change="handleReAssignFeedback('main_job_career_1_year_from', $event)"
                                          />

                                          <span style="font-size: 12px;" class="ml-2">{{ $t('HR_LIST_FORM.YEAR') }}</span>
                                        </div>

                                        <b-form-invalid-feedback :state="error.main_job_career_1_year_from">
                                          <span style="font-size: 10px;">{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                        </b-form-invalid-feedback>
                                      </div>

                                      <div class="w-100 d-flex flex-column" style="width: 46%; gap: 0.25rem; ">
                                        <div class="w-100 d-flex flex-row align-items-center">
                                          <b-form-select
                                            v-model="main_job_career_1_month_from"
                                            :options="date_of_birth_month_option"
                                            style="width: 60px;"
                                            :class="error.main_job_career_1_month_from === false ? 'is-invalid' : ''"
                                            @change="handleReAssignFeedback('main_job_career_1_month_from', $event)"
                                          />

                                          <span style="font-size: 12px;" class="ml-2">{{ $t('HR_LIST_FORM.MONTH') }}</span>
                                        </div>

                                        <b-form-invalid-feedback :state="error.main_job_career_1_month_from">
                                          <span style="font-size: 10px;">{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                        </b-form-invalid-feedback>
                                      </div>
                                    </div>
                                    ...
                                    <div class="w-100 d-flex justify-space-between align-center" style="gap: 0.5rem;">
                                      <div class="w-100 d-flex flex-column" style="width: 48%; gap: 0.25rem;">
                                        <div class="w-100 d-flex flex-row align-items-center">
                                          <b-form-select
                                            v-model="main_job_career_1_year_to"
                                            :options="date_of_birth_year_option"
                                            style="width: 80px;"
                                            :disabled="main_job_career_1_time_now === true"
                                            :class="error.main_job_career_1_year_to === false ? 'is-invalid' : ''"
                                            @change="handleReAssignFeedback('main_job_career_1_year_to', $event)"
                                          />

                                          <span style="font-size: 12px;" class="ml-2">{{ $t('HR_LIST_FORM.YEAR') }}</span>
                                        </div>

                                        <b-form-invalid-feedback :state="error.main_job_career_1_year_to">
                                          <span style="font-size: 10px;">{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                        </b-form-invalid-feedback>
                                      </div>
                                      ...
                                      <div class="w-100 d-flex flex-column" style="width: 48%; gap: 0.25rem;">
                                        <div class="w-100 d-flex flex-row align-items-center">
                                          <b-form-select
                                            v-model="main_job_career_1_month_to"
                                            :options="date_of_birth_month_option"
                                            style="width: 60px;"
                                            :disabled="main_job_career_1_time_now === true"
                                            :class="error.main_job_career_1_month_to === false ? 'is-invalid' : ''"
                                            @change="handleReAssignFeedback('main_job_career_1_month_to', $event)"
                                          />

                                          <span style="font-size: 12px;" class="ml-2">{{ $t('HR_LIST_FORM.MONTH') }}</span>
                                        </div>

                                        <b-form-invalid-feedback :state="error.main_job_career_1_month_to">
                                          <span style="font-size: 9px;">{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                        </b-form-invalid-feedback>
                                      </div>
                                    </div>
                                  </div>

                                  <div class="d-flex justify-end align-center" style="min-width: 12%; gap: 4px;">
                                    <b-form-checkbox
                                      id="main-job-career-1"
                                      v-model="main_job_career_1_time_now"
                                      name="main-job-career-1"
                                      @change="clearValueWhenDisable($event, 'main-job-career-1')"
                                    >
                                      <span style="font-size: 12px;">現在</span>
                                    </b-form-checkbox>
                                  </div>
                                </div>
                              </div>
                            ...
                            <div class="w-100 d-flex justify-space-center align-center">
                              ...
                              <div class="w-100 d-flex flex-column justify-space-between align-center">
                                <b-form-select
                                  v-model="main_job_career_1_department"
                                  :options="deparment_options_1"
                                  :class="error.main_job_career_1_department === false ? 'is-invalid' : ''"
                                  @change="handleReAssignFeedback('main_job_career_1_department', $event)"
                                />

                                <b-form-invalid-feedback :state="error.main_job_career_1_department">
                                  <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                </b-form-invalid-feedback>
                              </div>
                            </div>
                            ...
                            <div class="w-100 d-flex justify-space-center align-center">
                              ...
                              <div class="w-100 d-flex flex-column justify-space-between align-center">
                                <b-form-select
                                  ...
                                  :disabled="main_job_career_1_department === null"
                                  :class="error.main_job_career_1_job_title === false ? 'is-invalid' : ''"
                                  @change="handleReAssignFeedback('main_job_career_1_job_title', $event)"
                                />

                                <b-form-invalid-feedback :state="error.main_job_career_1_job_title">
                                  <span>{{ $t('VALIDATE.REQUIRED_TEXT') }}</span>
                                </b-form-invalid-feedback>
                              </div>
                            </div>
                            ...
                            <div class="w-100 d-flex justify-space-center align-center">
                              <div class="w-100 d-flex justify-space-between align-center">
                                ...
                                <div class="w-100 d-flex flex-row justify-content-center align-items-center" style="gap: 0.75rem;">
                                  <div class="w-100 d-flex justify-start align-center">
                                    <div class="w-100 d-flex justify-space-between align-center" style="gap: 0.5rem;">
                                      <div class=" d-flex justify-space-between align-center" style="width: 54%; gap: 0.25rem;">
                                        <b-form-select
                                          v-model="main_job_career_2_year_from"
                                          :options="date_of_birth_year_option"
                                        />

                                        <span style="font-size: 12px;">{{ $t('HR_LIST_FORM.YEAR') }}</span>
                                      </div>

                                      <div class=" d-flex justify-space-between align-center" style="width: 46%; gap: 0.25rem;">
                                        <b-form-select
                                          v-model="main_job_career_2_month_from"
                                          :options="date_of_birth_month_option"
                                        />

                                        <span style="font-size: 12px;">{{ $t('HR_LIST_FORM.MONTH') }}</span>
                                      </div>
                                    </div>
                                    ...
                                    <div class="w-100 d-flex justify-space-between align-center" style="gap: 0.5rem;">
                                      <div class=" d-flex justify-space-between align-center" style="width: 54%; gap: 0.25rem;">
                                        <b-form-select
                                          v-model="main_job_career_2_year_to"
                                          :options="date_of_birth_year_option"
                                          :disabled="main_job_career_2_time_now === true"
                                        />
                                      ...
                                      <div class=" d-flex justify-space-between align-center" style="width: 46%; gap: 0.25rem;">
                                        <b-form-select
                                          v-model="main_job_career_2_month_to"
                                          :options="date_of_birth_month_option"
                                          :disabled="main_job_career_2_time_now === true"
                                        />
                                  ...
                                  <div class="d-flex justify-end align-center" style="min-width: 12%; gap: 4px;">
                                    <b-form-checkbox
                                      ...
                                      :value="true"
                                      @click="clearValueWhenDisable($event, 'main-job-career-2')"
                                    >
                                      <span style="font-size: 12px;">現在</span>
                                    </b-form-checkbox>
                                  </div>
                            ...
                            <div class="w-100 d-flex justify-space-center align-center">
                              ...
                              <div class="w-100 d-flex justify-space-between align-center">
                                <b-form-select
                                  ...
                                  :disabled="main_job_career_2_department === null"
                                />
                              </div>
                            </div>
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r border-b">
                        <div id="hr-type-edit" class="hr-content-tab-item__title d-flex justify-start align-start">
                          <span style="font-size: 12px;">{{ $t('HR_LIST_FORM.MAIN_JOB_CAREER_3') }}</span>
                          <Arbitrarily />
                        </div>
                                ...
                                <div class="w-100 d-flex flex-row justify-content-center align-items-center" style="gap: 0.75rem; ">
                                  <div class="w-100 d-flex justify-start align-center">
                                    <div class="w-100 d-flex justify-space-between align-center" style="gap: 0.5rem;">
                                      <div class="d-flex justify-space-between align-center" style="width: 54%; gap: 0.25rem; ">
                                        <b-form-select
                                          v-model="main_job_career_3_year_from"
                                          :options="date_of_birth_year_option"
                                        />
                                      ...
                                      <div class="d-flex justify-space-between align-center" style="width: 46%; gap: 0.25rem; ">
                                        <b-form-select
                                          v-model="main_job_career_3_month_from"
                                          :options="date_of_birth_month_option"
                                        />
                                    ...
                                    <div class="w-100 d-flex justify-space-between align-center" style="gap: 0.5rem;">
                                      <div class="d-flex justify-space-between align-center" style="width: 54%; gap: 0.25rem;">
                                        <b-form-select
                                          v-model="main_job_career_3_year_to"
                                          :options="date_of_birth_year_option"
                                          :disabled="main_job_career_3_time_now === true"
                                        />
                                      ...
                                      <div class="d-flex justify-space-between align-center" style="width: 46%; gap: 0.25rem;">
                                        <b-form-select
                                          v-model="main_job_career_3_month_to"
                                          :options="date_of_birth_month_option"
                                          :disabled="main_job_career_3_time_now === true"
                                        />
                                  ...
                                  <div class="d-flex justify-end align-center" style="min-width: 12%; gap: 4px;">
                                    <b-form-checkbox
                                      ...
                                      :value="true"
                                      @click="clearValueWhenDisable($event, 'main-job-career-3')"
                                    >
                                      <span style="font-size: 12px;">現在</span>
                                    </b-form-checkbox>
                                  </div>
                            ...
                            <div class="w-100 d-flex justify-space-center align-center">
                              ...
                              <div class="w-100 d-flex justify-space-between align-center">
                                <b-form-select
                                  ...
                                  :disabled="main_job_career_3_department === null"
                                />
                              </div>
                            </div>
              ...
              <b-tab title="自己PR・備考" :title-link-class="isPassThirdTabValidation ? '' : 'error'">
                <b-card-text>
                  <div class="hr-content-tab">
                    <div class="hr-content-tab-wrap">
                      <div class="hr-content-tab-item border-t border-l border-r" style="min-height: 240px">
                        <div class="hr-content-tab-item__title d-flex justify-content-start align-items-start">
                          <span>{{ $t('HR_LIST_FORM.PERSONAL_PR_SPECIAL_NOTES') }}</span>
                          <Arbitrarily />
                        </div>

                        <div class="hr-content-tab__data d-flex justify-content-start align-items-start py-2">
                          <div class="w-100 h-100 d-flex justify-content-start align-items-stretch">
                            <b-form-textarea
                              id="textarea"
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r border-b" style="min-height: 240px">
                        <div class="hr-content-tab-item__title d-flex justify-content-start align-items-start">
              ...
              <b-tab title="連絡先" :title-link-class="isPassFourthTabValidation ? '' : 'error'">
                <b-card-text>
                  <div class="hr-content-tab">
                    <div class="hr-content-tab-wrap">
                      <div class="hr-content-tab-item  border-t border-l border-r">
                        <div class="hr-content-tab-item__title">
                          <span>{{ $t('HR_LIST_FORM.TELEPHONE_NUMBER') }}</span>
                          <Arbitrarily />
                        </div>

                        <div class="hr-content-tab__data ">
                          <div class="flex-row" style="gap: 1rem; justify-content: flex-start; align-items: center;">
                            <b-form-select v-model="telephone_phone_number_prefix" :options="phone_number_options" style="min-width: 153px; width: 28%;" />
                      ...
                      <div class="hr-content-tab-item  border-t border-l border-r">
                        ...
                        <div class="hr-content-tab__data ">
                          <div class="flex-row " style="gap: 1rem; justify-content: flex-start; align-items: center;">
                            <b-form-select v-model="mobile_phone_number_prefix" :options="phone_number_options" style="min-width: 153px; width: 28%;" />
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r">
                        ...
                        <div class="hr-content-tab__data">
                          <div class="w-100 d-flex flex-column" style="justify-content: flex-start; align-items: center;">
                            <b-form-input
                              ...
                              :class="!error.mail_address ? 'mt-3 is-invalid' : ''"
                              @input="handleReAssignFeedback('mail_address', $event)"
                            />
                      ...
                      <div class="hr-content-tab-item border-t border-l border-r">
                        <div class="hr-content-tab-item__title d-flex" style="justify-content: flex-start; align-items: flex-start; padding-top: 1.5rem;">
                          <span>{{ $t('HR_LIST_FORM.ADDRESS') }}</span>
                          <Arbitrarily />
                        </div>

                        <div class="hr-content-tab__data ">
                          <div class="flex-row" style="gap: 1rem; justify-content: flex-start; align-items: center; padding: 1rem 0;">
                            <div class="w-100 d-flex  flex-column  justify-space-between align-center" style="gap: 1.751rem;">
                              <div class="w-100 d-flex" style="height: 48px; justify-content: space-between; align-items: center; gap: 0.751rem;">
                                <span style="min-width: 15%;">{{ $t('HR_LIST_FORM.CITY') }}</span>
                                <b-form-input v-model="address_city" max-lenght="50" :formatter="format50characters" style="width: 80%;" />
                              </div>

                              <div class="w-100 d-flex" style="height: 48px; justify-content: space-between; align-items: center; gap: 0.751rem;">
                                <span style="min-width: 15%;">{{ $t('HR_LIST_FORM.DISTRICT') }}</span>
                                <b-form-input v-model="address_district" max-lenght="50" :formatter="format50characters" style="width: 80%;" />
                              </div>

                              <div class="w-100 d-flex" style="height: 48px; justify-content: space-between; align-items: center; gap: 0.751rem;">
                                <span style="min-width: 15%;">{{ $t('HR_LIST_FORM.NUMBER') }}</span>
                                <b-form-input v-model="address_number" max-lenght="50" :formatter="format50characters" style="width: 80%;" />
                              </div>

                              <div class="w-100 d-flex" style="height: 48px; justify-content: space-between; align-items: center; gap: 0.751rem;">
                                <span style="min-width: 15%;">{{ $t('HR_LIST_FORM.ORTHER') }}</span>
                                <b-form-input v-model="address_other" max-lenght="50" :formatter="format50characters" style="width: 80%;" />
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="hr-content-tab__data">
                          <div class="flex-row" style="gap: 1rem; justify-content: flex-start; align-items: center; padding: 1rem 0">
                            <div class="w-100 d-flex flex-column justify-space-between align-center" style="gap: 1.751rem;">
                              <div class="w-100 d-flex" style="height: 48px; justify-content: space-between;align-items: center; gap: 0.751rem;">
                                <span style="min-width: 15%;">{{ $t('HR_LIST_FORM.CITY') }}</span>
                                <b-form-input v-model="hometown_address_city" max-lenght="50" :formatter="format50characters" style="width: 80%;" />
                              </div>

                              <div class="w-100 d-flex" style="height: 48px; justify-content: space-between;align-items: center; gap: 0.751rem;">
                                <span style="min-width: 15%;">{{ $t('HR_LIST_FORM.DISTRICT') }}</span>
                                <b-form-input v-model="hometown_address_district" max-lenght="50" :formatter="format50characters" style="width: 80%;" />
                              </div>

                              <div class="w-100 d-flex" style="height: 48px; justify-content: space-between;align-items: center; gap: 0.751rem;">
                                <span style="min-width: 15%;">{{ $t('HR_LIST_FORM.NUMBER') }}</span>
                                <b-form-input v-model="hometown_address_number" max-lenght="50" :formatter="format50characters" style="width: 80%;" />
                              </div>

                              <div class="w-100 d-flex" style="height: 48px; justify-content: space-between;align-items: center; gap: 0.751rem;">
                                <span style="min-width: 15%;">{{ $t('HR_LIST_FORM.ORTHER') }}</span>
                                <b-form-input v-model="hometown_address_other" max-lenght="50" :formatter="format50characters" style="width: 80%;" />
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <b-tab title="マッチング状況" :title-link-class="isPassFifthTabValidation ? '' : 'error'">
                <b-card-text>
                  <div class="hr-detail-matching-situation-tab">
                    <div class="hr-detail-list-status-process">
                      <div class="hr-detail-list-status-process-item border-t border-l border-r">
                        ...
                        <div class="hr-detail-list-status-process--content">
                          <div class="hr-detail-list-status">
                            <div v-for="(item, index) in entries" :key="index" class="hr-detail-list-status-item">
                              <div class="hr-detail-list-status-item__status">
                                <div :class="item.status" class="hr-status-block">
                                  <span class="one-line-paragraph">{{ handleRenderTextByStatus(1, item.origin_status) }}</span>
                                </div>
                              </div>

                              <div class="hr-detail-list-status-item__candidate-info">
                                <span class="one-line-paragraph">{{ item.remarks }}</span>
                              </div>

                              <div class="hr-detail-list-status-item__entry-date">
                                <div>{{ $t('HR_LIST.ENTRY_DATE') }}：</div>
                                <span class="one-line-paragraph">{{ item.entry_date }}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="hr-detail-list-status-process-item border-t border-l border-r">
                        ...
                        <div class="hr-detail-list-status-process--content">
                          <div class="hr-detail-list-status">
                            <div v-for="(item, index) in offers" :key="index" class="hr-detail-list-status-item">
                              <div class="hr-detail-list-status-item__status">
                                <div :class="item.status" class="hr-status-block">
                                  <span class="one-line-paragraph">{{ handleRenderTextByStatus(2, item.origin_status) }}</span>
                                </div>
                              </div>
                      ...
                      <div class="hr-detail-list-status-process-item border-t border-l border-r">
                        ...
                        <div class="hr-detail-list-status-process--content">
                          <div class="hr-detail-list-status">
                            <div v-for="(item, index) in interviews" :key="index" class="hr-detail-list-status-item">
                              <div class="hr-detail-list-status-item__status">
                                <div :class="item.status" class="hr-status-block">
                                  <span class="one-line-paragraph">{{ handleRenderTextByStatus(3, item.origin_status) }}</span>
                                </div>
                              </div>
                      ...
                      <div class="hr-detail-list-status-process-item border-t border-l border-r border-b">
                        ...
                        <div class="hr-detail-list-status-process--content">
                          <div class="hr-detail-list-status">
                            <div v-for="(item, index) in results" :key="index" class="hr-detail-list-status-item">
                              <div class="hr-detail-list-status-item__status">
                                <div :class="item.status" class="hr-status-block">
                                  <span class="one-line-paragraph">{{ handleRenderTextByStatus(4, item.origin_status) }}</span>
                                </div>
                              </div>
...
export default {
  ...
  computed: {
    job_title_manufacturingy_1() {
      ...
      // const ORINAL_OPTIONS = [...job_title_manufacturingy_api];
      ...
      JOB_TITLE_MANUFACTURINGY_API.unshift({ value: null, text: '選択してください', disabled: true });
    ...
    deparment_options_1() {
      ...
      DEPARMENT_OPTION_API.unshift({ value: null, text: '選択してください', disabled: true });
    ...
    middle_classification_options_api() {
      ...
      MIDDLE_CLASSIFICATION_OPTIONS_API.unshift({ value: null, text: '選択してください', disabled: true });
    ...
    major_classification_options_api() {
      ...
      MAJOR_CLASSIFICATION_OPTIONS_API.unshift({ value: null, text: '選択してください', disabled: true });
    ...
    final_education_degree_options() {
      ...
      FINAL_EDUCATION_DEGREE_OPTIONS.unshift({ value: null, text: '選択してください', disabled: true });
    ...
    final_education_classification_options() {
      ...
      FINAL_EDUCATION_CLASSIFICATION_OPTIONS.unshift({ value: null, text: '選択してください', disabled: true });
    ...
    phone_number_options() {
      ...
      PHONE_NUMBER_OPTIONS.unshift({ value: null, text: '選択してください', disabled: true });
  ...
  methods: {
      ...
      if (this.HR_ID) {
        ...
        try {
          ...
          if (response['code'] === 200) {
            ...
            this.original_hr_id = DATA['id'];
            this.country_id = DATA['country_id'];

            this.full_name = DATA['full_name'];
            this.full_name_furigana = DATA['full_name_ja'];
            this.gender = DATA['gender'];
            this.date_of_birth_year = this.handleProcessDoB(DATA['date_of_birth'])[0];
            this.date_of_birth_month = this.handleProcessDoB(DATA['date_of_birth'])[1];

            await this.handleRenderDayInMonth(this.date_of_birth_year, this.date_of_birth_month);

            this.date_of_birth_day = this.handleProcessDoB(DATA['date_of_birth'])[2];
            ...
            this.final_education_timing_year = this.handleGetMonthDayInDate(DATA['final_education_date'])[0];
            this.final_education_timing_month = this.handleGetMonthDayInDate(DATA['final_education_date'])[1];
            this.final_education_classification = DATA['final_education_classification'];
            ...
            if (DATA['main_jobs'][0]) {
              this.main_job_career_1_year_from = this.handleGetMonthDayInDate(DATA['main_jobs'][0]['main_job_career_date_from'])[0];
              this.main_job_career_1_month_from = this.handleGetMonthDayInDate(DATA['main_jobs'][0]['main_job_career_date_from'])[1];

              this.main_job_career_1_time_now = false;
              this.main_job_career_1_year_to = this.handleGetMonthDayInDate(DATA['main_jobs'][0]['main_job_career_date_to'])[0];
              this.main_job_career_1_month_to = this.handleGetMonthDayInDate(DATA['main_jobs'][0]['main_job_career_date_to'])[1];

              this.main_job_career_1_department = DATA['main_jobs'][0]['department_id'];
            ...
            if (DATA['main_jobs'][1] !== undefined) {
              this.main_job_career_2_year_from = this.handleGetMonthDayInDate(DATA['main_jobs'][1]['main_job_career_date_from'])[0];
              this.main_job_career_2_month_from = this.handleGetMonthDayInDate(DATA['main_jobs'][1]['main_job_career_date_from'])[1];

              this.main_job_career_2_department = DATA['main_jobs'][1]['department_id'];
              ...
              this.main_job_career_2_year_to = this.handleGetMonthDayInDate(DATA['main_jobs'][1]['main_job_career_date_to'])[0];
              this.main_job_career_2_month_to = this.handleGetMonthDayInDate(DATA['main_jobs'][1]['main_job_career_date_to'])[1];
            }

            if (DATA['main_jobs'][2] !== undefined) {
              this.main_job_career_3_year_from = this.handleGetMonthDayInDate(DATA['main_jobs'][2]['main_job_career_date_from'])[0];
              this.main_job_career_3_month_from = this.handleGetMonthDayInDate(DATA['main_jobs'][2]['main_job_career_date_from'])[1];

              this.main_job_career_3_time_now = false;
              this.main_job_career_3_year_to = this.handleGetMonthDayInDate(DATA['main_jobs'][2]['main_job_career_date_to'])[0];
              this.main_job_career_3_month_to = this.handleGetMonthDayInDate(DATA['main_jobs'][2]['main_job_career_date_to'])[1];

              this.main_job_career_3_department = DATA['main_jobs'][2]['department_id'];
              this.main_job_career_3_job_title = DATA['main_jobs'][2]['job_id'];
              this.main_job_career_3_textarea = DATA['main_jobs'][2]['detail'];
            }

            this.personal_pr_special_textarea = DATA['personal_pr_special_notes'];
            this.remarks_textarea = DATA['remarks'];

            this.telephone_phone_number_prefix = this.handlePhoneNumber(DATA['telephone_number'])[0];
            this.telephone_phone_number = this.handlePhoneNumber(DATA['telephone_number'])[1];

            this.mobile_phone_number_prefix = this.handlePhoneNumber(DATA['mobile_phone_number'])[0];
            this.mobile_phone_number = this.handlePhoneNumber(DATA['mobile_phone_number'])[1];
    ...
    async handleSaveHr() {
      if (this.handleValidateFormData()) {
        const MAIN_JOBS = [
          {
            main_job_career_date_from: `${this.main_job_career_1_year_from}-${this.handleFormatMonthDay(this.main_job_career_1_month_from)}`,
        ...
        if (this.main_job_career_2_year_from && this.main_job_career_2_month_from) {
          MAIN_JOBS.push(
            {
              main_job_career_date_from: `${this.main_job_career_2_year_from}-${this.handleFormatMonthDay(this.main_job_career_2_month_from)}`,
              to_now: this.main_job_career_2_time_now ? 'yes' : 'no',
              main_job_career_date_to: `${this.main_job_career_2_year_to}-${this.handleFormatMonthDay(this.main_job_career_2_month_to)}`,
              department_id: this.main_job_career_2_department,
              job_id: this.main_job_career_2_job_title,
              detail: this.main_job_career_2_textarea,
            }
          );
        }

        if (this.main_job_career_3_year_from && this.main_job_career_3_month_from) {
          MAIN_JOBS.push(
            {
              main_job_career_date_from: `${this.main_job_career_3_year_from}-${this.handleFormatMonthDay(this.main_job_career_3_month_from)}`,
              to_now: this.main_job_career_3_time_now ? 'yes' : 'no',
              main_job_career_date_to: `${this.main_job_career_3_year_to}-${this.handleFormatMonthDay(this.main_job_career_3_month_to)}`,
              department_id: this.main_job_career_3_department,
              job_id: this.main_job_career_3_job_title,
              detail: this.main_job_career_3_textarea,
            }
          );
        }
        ...
        const DATA = {
          ...
          gender: this.gender,
          date_of_birth: `${this.date_of_birth_year}-${this.handleFormatMonthDay(this.date_of_birth_month)}-${this.handleFormatMonthDay(this.date_of_birth_day)}`,
          work_form: this.work_form,
          preferred_working_hours: this.work_form_part_time,
          japanese_level: this.japanese_level,

          final_education_date: `${this.final_education_timing_year}-${this.handleFormatMonthDay(this.final_education_timing_month)}`,
          ...
          telephone_number: this.handleMergePhoneNumber(this.telephone_phone_number_prefix, this.telephone_phone_number),
          mobile_phone_number: this.handleMergePhoneNumber(this.mobile_phone_number_prefix, this.mobile_phone_number),
        ...
        const URL = `${urlAPI.urlEditHr}${this.original_hr_id}`;
        ...
        if (response['code'] === 200) {
          ...
          MakeToast({
            ...
            content: '[ERROR RESPONSE API UPDATE]',
          });
        }
      } else {
        MakeToast({
          variant: 'warning',
          title: this.$t('WARNING'),
          content: 'フォームデータの検証に失敗しました。必須項目を入力してください。',
        });
      }
    },
    handleValidateFormData() {
      let result_first_tab = false;
      let result_second_tab = false;
      const result_third_tab = true;
      let result_fourth_tab = false;
      let result_hr_information = false;

      if (this.full_name === '') {
        this.error.full_name = false;
        this.isPassFirstTabValidation = false;
      } else if (this.full_name_furigana === '') {
        this.error.full_name_furigana = false;
        this.isPassFirstTabValidation = false;
      } else if (this.date_of_birth_year === '') {
        this.error.date_of_birth_year = false;
        this.isPassFirstTabValidation = false;
      } else if (this.date_of_birth_month === '') {
        this.error.date_of_birth_month = false;
        this.isPassFirstTabValidation = false;
      } else if (this.date_of_birth_day === '') {
        this.error.date_of_birth_day = false;
        this.isPassFirstTabValidation = false;
      } else if (this.japanese_level === null) {
        this.error.japanese_level = false;
        this.isPassFirstTabValidation = false;
      } else {
        result_first_tab = true;
        this.isPassFirstTabValidation = true;
      }

      if (this.final_education_timing_year === null) {
        this.error.final_education_timing_year = false;
        this.isPassSecondTabValidation = false;
      } else if (this.final_education_timing_month === null) {
        this.error.final_education_timing_month = false;
        this.isPassSecondTabValidation = false;
      } else if (this.final_education_classification === null) {
        this.error.final_education_classification = false;
        this.isPassSecondTabValidation = false;
      } else if (this.final_education_degree === null) {
        this.error.final_education_degree = false;
        this.isPassSecondTabValidation = false;
      } else if (this.major_classification === null) {
        this.error.major_classification = false;
        this.isPassSecondTabValidation = false;
      } else if (this.middle_classification === null) {
        this.error.middle_classification = false;
        this.isPassSecondTabValidation = false;
      } else if (this.main_job_career_1_year_from === null) {
        this.error.main_job_career_1_year_from = false;
        this.isPassSecondTabValidation = false;
      } else if (this.main_job_career_1_month_from === null) {
        this.error.main_job_career_1_month_from = false;
        this.isPassSecondTabValidation = false;
      } else if (this.main_job_career_1_time_now === false && this.main_job_career_1_year_to === null) {
        this.error.main_job_career_1_year_to = false;
        this.isPassSecondTabValidation = false;
      } else if (this.main_job_career_1_time_now === false && this.main_job_career_1_month_to === null) {
        this.error.main_job_career_1_month_to = false;
        this.isPassSecondTabValidation = false;
      } else if (this.main_job_career_1_department === null) {
        this.error.main_job_career_1_department = false;
        this.isPassSecondTabValidation = false;
      } else if (this.main_job_career_1_job_title === null) {
        this.error.main_job_career_1_job_title = false;
        this.isPassSecondTabValidation = false;
      } else {
        result_second_tab = true;
        this.isPassSecondTabValidation = true;
      }

      if (this.mail_address === '') {
        this.error.mail_address = false;
        this.isPassFourthTabValidation = false;
        this.invalid_mail_error_text = '入力してください。';
      } else if (this.mail_address && !this.isValidEmail(this.mail_address)) {
        this.error.mail_address = false;
        this.isPassFourthTabValidation = false;
        this.invalid_mail_error_text = 'メールアドレスの形式が正しくありませんでした。';
      } else {
        result_fourth_tab = true;
        this.isPassFourthTabValidation = true;
      }

      if (this.cv_file === null) {
        this.error.cv_file = false;
      } else if (this.jd_file === null) {
        this.error.jd_file = false;
      } else {
        result_hr_information = true;
      }

      if (result_first_tab && result_second_tab && result_third_tab && result_fourth_tab && result_hr_information) {
        return true;
      }
    },
    calculateAge(dob) {
    ...
    handleFileSelect(type, event, index) {
      ...
      if (SELECTED_FILE) {
        ...
        const FILE_NAME = `${this.shortenString(SELECTED_FILE['name'])} (${this.bytesToMB(SELECTED_FILE['size'])}MB)`;
        ...
        switch (type) {
          ...
          case 3:
            ...
            this.handleUploadSingleFile(3, this.default_other_file, this.default_other_file_name);
          ...
          case 4:
            ...
            this.handleUploadSingleFile(4, this.other_files[index]['file'], this.other_files[index]['file_name'], index);
            break;
          default:
            break;
        }
    ...
    handlePreview(type, index) {
      const LIST_FILE = [this.cv_file_preview_src, this.jd_file_preview_src, this.default_other_file_preview_src];
    ...
    handleAddOtherFile() {
      this.other_files.push(
        { file: null, file_name: '選択されていません', file_preview_url: '', file_id: '', file_path: '', system_file_name: '' }
      );
    },
        ...
        if (response['code'] === 200) {
          switch (type) {
            ...
            case 3:
              ...
              this.default_other_file_system_file_name = response['data']['file_name'];
              break;
            case 4:
              this.other_files[index]['file_id'] = response['data']['id'];
              this.other_files[index]['file_path'] = response['data']['file_path'];
              this.other_files[index]['system_file_name'] = response['data']['file_name'];
              break;
            default:
              break;
          }
        } else {
          console.log('[ERROR RESPONSE API UPLOAD SINGLE FILE]');
        }
      } catch (error) {
        console.log(error);
      }
    ...
    handleReAssignFeedback(field, value) {
      ...
      } else if (field === 'main_job_career_1_year_from') {
        if (value) {
    ...
    isValidEmail(email) {
      const emailRegex = /^[a-zA-Z0-9._%+-]{1,50}@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/;
