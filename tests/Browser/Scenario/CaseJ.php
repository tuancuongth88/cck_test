...
class CaseJ extends DuskTestCase
{
    private $faker;
    ...
    private function fakeDataInterview()
    {
        ...
        foreach ($hrs as $key1 => $hr) {
            $interview = Interview::create([
                ...
                Interview::INTERVIEW_ADJUSTMENT => INTERVIEW_STATUS_INTERVIEW_ADJUSTMENT_URL_SETTING,
                Interview::REMARKS => '',
            ...
            InterviewInfo::create([
                ...
                InterviewInfo::STATUS => INTERVIEW_INFO_NUMBER_SECOND
            ]);
            $i++;
        }
    }

    public function caseSetZoomUrl(Browser $browser)
    {
        ...
        foreach ($types as $type) {
            ...
            $url = 'https://s.net.vn/WnNd';
            $zoom_id =  rand(11111111, 99999999);
            $password = Str::random(8);
            $browser->click('#interview-table tbody tr:nth-child(' . $i . ') .btn-go-detail')->pause(5000)
                ->assertSee('《エントリー》')
                ->assertSee('一次面接')
                ->assertSee('個人面接')
                ->type('@url_zoom_text', $url)->pause(500)
                ->type('@id_zoom_text', $zoom_id)->pause(500)
                ->type('@password_zoom_text', $password)->pause(500)
                ->click('#URL-setting-modal > div.content-detail.px-5 > div:nth-child(5) > button')->pause(5000)
                ->assertSeeIn('#interview-table > tbody > tr:nth-child(' . $i . ') > td:nth-child(3)', 'E' . $entry_code)
                ->assertSeeIn('#interview-table > tbody > tr:nth-child(' . $i . ') > td:nth-child(7) > span', '書類通過')
            ...
            $browser->click('#nav-collapse > ul > li:nth-child(1) > a')->pause(5000)
                ->assertSee('Zoom URLの発行が完了しました')->pause(4000)
            ...
            $typeOther = [\HR, COMPANY_MANAGER, \COMPANY];
            foreach ($typeOther as $other) {
                $this->login($other);
                $browser->pause(5000)
                    ->assertPathIs('/home')
                    ->assertSee('Zoom URLの発行が完了しました')->pause(4000)
                ...
                $browser->click('#interview-table tbody tr:nth-child(' . $i . ') .btn-go-detail')->pause(5000)
//                    ->assertSee('')
                    ->assertSee('ミーティングID：'.$zoom_id)
                    ->assertSee('パスコード：'.$password);
