...
class SelectOfferTest extends DuskTestCase
{
    /**
     * A Dusk test example.
     *
    ...
    private function caseListOffer($browser)
    {
        $browser->pause(3000)
            ->mouseover('.display-menu > li:nth-child(2)')->pause(2000)
            ->visit('hr/list')->pause(3000)
            ->assertSee('人材一覧')
            ->assertSee('ダウンロード')->pause(1000)
            ->assertButtonEnabled('#hr-table-list > tbody > tr:nth-child(1) > td.detail > div')
            ->click('#hr-table-list > tbody > tr:nth-child(1) > td.detail > div')->pause(4000)
            ->assertSee('人材情報詳細')->pause(1000)
            ->assertSee('オファーする')->pause(1000)
            ->assertButtonEnabled('#offer')
            ->click('#offer')->pause(3000)
            ->assertButtonEnabled('tbody > tr:nth-child(1)')
            ->click('tbody > tr:nth-child(1)')->pause(3000)
            ->within('table', function ($table) {
                $table->scrollIntoView('td:last-child');
            })->pause(3000)
            ->assertButtonEnabled('.btn-confirm-offer')
            ->click('.btn-confirm-offer')->pause(5000)
            ->assertSee('オファーしてよろしいですか？')
            ->assertSee('オファー求人')
            ->assertSee('オファー人')
            ->assertButtonEnabled('.btn-ok')
            ->click('.btn-ok')->pause(1000)
            ->assertSee('オファーしました')
            ->pause(2000)
            ->assertButtonEnabled('.btn-cancel-offer')
            ->click('.btn-cancel-offer')->pause(2000)
            ->visit('/matching-management')->pause(3000)
            ->click('.nav-tabs li:nth-child(2)')->pause(20000);
    }
